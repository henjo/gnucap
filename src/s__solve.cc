...
bool SIM::solve(OPT::ITL itl, TRACE trace)
{
  ...
  do{
    if (trace >= tITERATION) {
      print_results(static_cast<double>(-iteration_number()));
    }
    set_flags();
    clear_arrays();
...
{
  if (!is_inc_mode()) {			/* Clear working array */
    SIM::aa.zero();
...
void SIM::evaluate_models()
{
  ...
  while (!late_evalq.empty()) { //BUG// encapsulation violation
    converged &= late_evalq.front()->do_tr_last();
...
{
  ::status.lud.start();
  lu.lu_decomp(aa, bool(OPT::lubypass && is_inc_mode()));
  ::status.lud.stop();
